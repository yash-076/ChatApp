    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
        ...
        <div className='flex flex-col'>
            {filteredUsers.map((user, index)=>(
                ...
                key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded-xl cursor-pointer max-sm-text-sm ${selectedUser ?._id === user._id && 'bg-[#282142]/50'} hover:bg-gray-400/30`}>
                    <img src={user?.profilePic || assets.avatar_icon} alt="ProfilePhoto" className='w-[35px] aspect-[1/1] rounded-full'/>
